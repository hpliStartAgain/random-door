...
- `AchievementService.Wall` 补 `user_id>0` 与用户存在性校验（`ensureUserExists` 经 db count，缺失返回脱敏 404）。
- `buildProgressStats` 不再吞 DB 错误：checkin 计数、城市 pluck、tag 查询、游戏访问计数任一失败都向上返回，由 handler 归为脱敏 500。
- `Wall` 现在传播 progress 统计错误（原 `stats, _ :=` 改为检查 err）。
- 成就解锁保持幂等：`achievement.Evaluate` 依赖 `user_achievements` 唯一约束，重复 Create 失败被跳过且不计入新解锁。
- 规则字典对齐保留：`first_checkin`、`checkin_count`、`city_tag`、`tag_count`、`game_visit_count`、`dice_distance`、`dice_direction`，未知规则返回 false。
- 新增 `achievement/rules_test.go`：18 个表驱动用例覆盖首打卡、计数达标/未达标/非法值、city_tag 命中/缺失、tag_count 达标/未达标/格式错/非数字、游戏访问、骰子距离、方向连击达标/不足/格式错、未知规则。

验证结果：

- 待最终统一验证阶段运行 `go test ./internal/achievement -run TestMatch` 等命令（本轮按老板要求暂不执行 build/test）。
- 静态自检：`Match` 分支与 seed 中的 `rule_type` 一一对应；`Wall` 错误路径接入 `writeServiceError` 的分类。

剩余风险：

- `Wall`/`Evaluate` 强依赖 `*gorm.DB`，未做离线 fake 单测；运行态正确性留到 Task 20 真实 DB 联调。
- 解锁仅靠唯一约束去重，未区分真实 DB 错误与重复键；MVP 可接受，最终审计复核。

下一步：

- 执行 Task 7：修缮 AI 对话编排与 chat API。

提交：

- 本 task 代码与测试在批量编码完成后统一提交。

### Checkpoint B [x] 完成 Task 4~6 后全面检查-debug循环
...
### Checkpoint C [x] 完成 Task 7~9 后全面检查-debug循环
...
### Task 11 [x] 修缮前端 API 封装与 Zustand 状态
...
### Checkpoint D [x] 完成 Task 10~12 后全面检查-debug循环
...
### Checkpoint E [x] 完成 Task 13~15 后全面检查-debug循环
...
### Checkpoint F [x] 完成 Task 16~18 后全面检查-debug循环
...
### Task 19 [x] 修缮 Docker Compose、环境模板与自动 seed
...
### Checkpoint G [x] 完成 Task 19~21 后全面检查-debug循环
...
### Checkpoint H [x] 完成 Task 22~24 后全面检查-debug循环
