...
- [ ] 新增 `frontend/src/lib/cityFilters.ts`
...
- [ ] 修改 `frontend/src/store/useCityStore.ts`
...
- [ ] 修改 `frontend/src/components/MapCanvas.tsx`
...
- [ ] 修改 `docs/design/api-contract.md`
  - `GET /api/cities` 的城市项增加 `landmark_count`、`food_count`、`character_count`。
- [ ] 修改 `docs/design/frontend-detailed-design.md`
  - 更新 `CityListItem` 与发现页职责。
- [ ] 修改 `docs/arch/backend-detailed-design.md`
  - 更新 `city_repo.go` 与 `city_service.go` 的聚合职责。
- [ ] 修改 `backend/internal/model/city.go`
  - 仅在响应 DTO 中增加统计字段，不建议把统计字段落库。
- [ ] 修改 `backend/internal/repository/city_repo.go`
  - 增加按城市聚合 landmarks / foods / characters 数量的查询。
- [ ] 修改 `backend/internal/service/city_service.go`
  - List 聚合统计字段。
- [ ] 修改 `backend/internal/api/city_handler.go`
  - 返回新字段。
- [ ] 修改 `frontend/src/api/types.ts`
  - `CityListItem` 增加三个 count 字段。
- [ ] 修改 `frontend/src/components/layout/Sidebar.tsx`
  - 展示当前筛选集合内的总地标、总美食、总人物。
- [ ] 修改 `backend/internal/service/city_service_test.go`
  - 覆盖统计字段。
...
- [ ] 修改 `frontend/src/components/CityDetailPanel.tsx`
...
- [ ] 修改 `frontend/src/components/RightDrawer.tsx`
...
- [ ] 修改 `docs/design/database-detailed-design.md`
  - `characters` 表增加 `role_title VARCHAR(128)`、`life_span VARCHAR(64)`、`intro_quote VARCHAR(255)`。
- [ ] 修改 `backend/migrations/schema.sql`
  - 同步新增字段。注意已有数据库升级需另配迁移说明，MVP seed 初始化可直接建表。
- [ ] 修改 `backend/internal/model/character.go`
  - 增加字段与 JSON tag，仍保持 `Persona` / `Prompt` 不下发。
- [ ] 修改 `backend/internal/repository/city_repo.go`
  - 查询并返回新增字段。
- [ ] 修改 `backend/internal/seed/seed.go`
  - seed 校验允许并校验新增字段长度。
- [ ] 修改 `backend/data/seed/cities.json`
  - 每个城市至少 1 个人物补 `role_title`、`life_span`、`intro_quote`。
- [ ] 修改 `frontend/src/api/types.ts`
  - `Character` 增加新增字段。
- [ ] 修改 `frontend/src/components/CityDetailPanel.tsx`
  - 按 yourwonder 的人物卡结构展示身份、年份、引用语。
- [ ] 修改 `backend/internal/model/schema_contract_test.go`
  - 覆盖新增字段。
- [ ] 修改 `backend/internal/seed/seed_test.go`
  - 覆盖新增字段校验。
...
- [ ] 新增 `frontend/src/components/GuessChallengeModal.tsx`
  - 展示截图预览、标题“猜猜我在哪”、生成文案、复制文案、保存图片、复制图片。
  - 使用现有 `api.generateGuessCaption()`。
- [ ] 新增 `frontend/src/lib/shareImage.ts`
  - 封装 canvas/blob/dataURL 转换。
  - 提供 `downloadImage()`、`copyImageToClipboard()`。
- [ ] 修改 `frontend/src/components/StreetViewCanvas.tsx`
  - 移出当前内联猜一猜面板的重复逻辑。
...
- [ ] 修改 `backend/migrations/schema.sql`
  - 新增两张表和索引。
- [ ] 新增 `backend/internal/model/guess_challenge.go`
- [ ] 新增 `backend/internal/model/guess_answer.go`
- [ ] 新增 `backend/internal/repository/guess_challenge_repo.go`
- [ ] 新增 `backend/internal/service/guess_challenge_service.go`
- [ ] 新增 `backend/internal/api/guess_challenge_handler.go`
- [ ] 修改 `backend/internal/api/router.go`
  - 注册挑战相关路由。
- [ ] 修改 `backend/internal/upload/storage.go`
...
- [ ] 新增 `frontend/src/pages/GuessChallengePage.tsx`
  - 展示截图，输入猜测，显示正确/接近/错误反馈。
- [ ] 修改 `frontend/src/App.tsx`
  - 如果现有项目继续无路由，可用 `useViewStore` 管理；如引入 route，必须与前端设计文档同步。
- [ ] 修改 `frontend/src/components/GuessChallengeModal.tsx`
  - `Challenge Friends` 生成链接并复制。
- [ ] 新增 `backend/internal/service/guess_challenge_service_test.go`
- [ ] 新增 `backend/internal/api/guess_challenge_handler_test.go`
...
- [ ] 新增 `frontend/src/components/ProfilePanel.tsx`
...
- [ ] 修改 `frontend/src/pages/AssetPage.tsx`
  - 保留完整资产页，但复用 ProfilePanel 的列表组件，避免两套 UI。
- [ ] 新增 `frontend/src/components/ProfileVisitedList.tsx`
  - 资产页和 ProfilePanel 共同复用。
- [ ] 新增 `frontend/src/components/ProfilePosterGrid.tsx`
  - 资产页和 ProfilePanel 共同复用。
...
- [ ] 新增 `backend/internal/service/user_service.go`
  - 处理 profile 读写校验。
- [ ] 新增 `backend/internal/api/user_handler.go`
  - 处理 profile 接口。
- [ ] 修改 `backend/internal/api/router.go`
  - 注册 profile 接口。
- [ ] 修改 `frontend/src/api/types.ts`
...
- [ ] 修改 `frontend/src/components/ProfilePanel.tsx`
  - 增加内联编辑。
- [ ] 新增 `backend/internal/service/user_service_test.go`
- [ ] 新增 `backend/internal/api/user_handler_test.go`
...
- [ ] 修改 `frontend/src/components/StreetViewCanvas.tsx`：将内联分享面板 JSX 抽到 `GuessChallengeModal.tsx`。
- [ ] 修改 `frontend/src/components/CityDetailPanel.tsx`：删除异常字符显示和过度说明文案。
...
- [ ] 如改 seed：`python scripts/build_seed.py validate`。
- [ ] 手动走通：进入首页、搜索/筛选城市、点击城市、进入地标视角、生成猜一猜文案、生成赛博打卡、查看个人足迹。
- [ ] 如改接口：先确认 `docs/design/api-contract.md` 已更新，再确认前后端字段一致。
